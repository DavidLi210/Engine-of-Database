...
public class Table{
		...
		try{
			...
			for(int page = 1; page <= numPages; page ++){
				...
				if(type == 0x05)
					...
					short[] cells = BtreePage.getCellArray(file, page);
					int i = 0;
					for(int j = 0; j < cells.length; j++){
						long loc = BtreePage.getCellLoc(file, page, j);
						String[] pl = retrievePayload(file, loc);
						String tb = pl[1];
						if(!tb.equals(table)){
							BtreePage.setCellOffset(file, page, i, cells[j]);
							i++;
						}
					}
					BtreePage.setCellNumber(file, page, (byte)i);
				}
			}
	...
	public static void createTable(String table, String[] col){
		try{	
			...
			for(int p = 1; p <= numPages; p++){
				int rm = BtreePage.getRightMost(file, p);
				if(rm == 0)
					page = p;
			}
			int[] keyArray = BtreePage.getKeyArray(file, page);
			int l = keyArray[0];
	...
	public static void update(String table, String[] set, String[] cmp){
		try{
			...
			for(int p = 1; p <= numPages; p++)
				if(BtreePage.hasKey(file, p, key)){
					page = p;
				}
			int[] array = BtreePage.getKeyArray(file, page);
			int id = 0;
			for(int i = 0; i < array.length; i++)
				if(array[i] == key)
					id = i;
			int offset = BtreePage.getCellOffset(file, page, id);
			long loc = BtreePage.getCellLoc(file, page, id);
			...
			BtreePage.updateLeafCell(file, page, offset, plsize, key, stc, values);
	...
	public static void insertInto(RandomAccessFile file, String table, String[] values){
		...
		if(page != 0)
			if(BtreePage.hasKey(file, page, key)){
		...
		int offset = BtreePage.checkLeafSpace(file, page, cellSize);


		if(offset != -1){
			BtreePage.insertLeafCell(file, page, offset, plSize, key, stc, values);
			//BtreePage.sortCellArray(file, page);
		}else{
			BtreePage.splitLeaf(file, page);
	...
	public static int searchKey(RandomAccessFile file, int key){
		...
		try{
			...
			for(int page = 1; page <= numPages; page++){
				...
				if(pageType == 0x0D){
					int[] keys = BtreePage.getKeyArray(file, page);
					if(keys.length == 0)
						return 0;
					int rm = BtreePage.getRightMost(file, page);
		...
		try{
			...
			for(int page = 1; page <= numPages; page++){
				...
				if(pageType == 0x05)
					...
					byte numCells = BtreePage.getCellNumber(file, page);

					for(int i=0; i < numCells; i++){
						//System.out.println("check point");
						long loc = BtreePage.getCellLoc(file, page, i);
